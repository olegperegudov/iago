...
- ⌘E on a screenshot opens it in Preview to draw on, and every save there comes
  back as a new clip — the original stays in the history behind it.
- The version in the tray menu opens the release list, where every version says
  what changed in it.

  **The menu-bar icon has two buttons now: left drops a panel, right opens a short menu.** Settings and the cheat sheet were two windows behind two menu items, so the menu carried five lines and the icon itself did nothing but open that menu. They are one panel now — a borderless card hanging under the icon, with a segmented row swapping the sides — and the menu is down to the update, the version, and quit. The split is what every menu-bar app does, and it is what freed the icon to be the way in.

  - **The segmented row, not tabs.** The app already swaps things with a segmented row (the retention choice), so the panel reuses that control literally: the same `.choice` class, with `aria-selected` driving the same look `.chosen` does. An underline-tab bar would have been a second idiom for the same job, in a 420px window that has room for exactly one.
  - **Tab is not a switch key.** The settings side has a checkbox, a slider and four buttons; taking Tab would leave nothing to walk between them with. The arrows switch while the cursor is on the row itself (roving tabindex, the tab pattern), and `⌘1` / `⌘2` reach a side from anywhere in the panel. `sides.js` is the decision, `sides.test.js` pins it — including that an arrow *inside* the pane belongs to the slider.
  - **Anchoring is arithmetic, so it is tested without a screen.** `tray::popover_position` centres the panel under the icon, flips it above when the icon sits on a bottom taskbar (Windows), and clamps it to the monitor. The clamp is written `.max(leftmost)` on purpose: on a screen narrower than the panel the naive clamp inverts and throws the window off the left edge — there is a test on exactly that.
  - **The click that closes it must not reopen it.** Clicking the icon takes focus off the panel, which hides it *before* the click handler runs; the handler would then see a hidden panel and put it straight back up. A 400 ms stamp written at every auto-hide (`tray::note_auto_hide`) is what makes the icon read as a toggle. Same mechanism on both platforms — the resign-key handler on macOS, the `Focused(false)` event on Windows.
  - **A second NSPanel, with different manners.** The popup must survive losing focus (it never has it, by design); the settings panel must vanish when it does. So `IagoPopover` is its own panel type: non-activating style mask like the popup, but ordinary window level (`is_floating_panel: false` — floating would park it over every other app forever), `MoveToActiveSpace` rather than joining all Spaces, and a `window_did_resign_key` handler that hides it.
  - **Two windows became one**, so `TRAY_WINDOWS` is a single label and the test that every configured window survives being closed now guards the panel. `settings.html` / `shortcuts.html` are gone; `settings.js` and `shortcuts.js` kept their logic and export `mountSettings(cfg)` / `mountShortcuts()`, which `panel.js` calls after a single `get_settings` — one round trip for the whole panel instead of one per window.
  - **Verified in the browser**, with the real panel.html and the Tauri bridge mocked: `web_eye/_iago_panel_check.mjs` drives the switch nine ways and drops the two screenshots the README uses. Instrument checked — dropping the `onSwitch` guard turns the arrow assertion red.
